...
########################## NEW PRICE HISTORY REQUEST METHOD ################
...
##############################################################################    
#################^^^^vvvv PICK ONE OR THE OTHER ##############################
###################### HOLD PRICE HISTORY REQUEST METHOD #####################
...
import time
count = 210
for i in range(21,len(symbol_chunks)):
    print('----------------------------------')
    ...
    for ticker in symbol_chunks[i]:
        ...
        # response = get_price_history(ticker, symbol=ticker, periodType='month', period='1', frequencyType='daily')
        

        recent_closes = []
        print(type(response))

        for bar in response['candles'][i]:
